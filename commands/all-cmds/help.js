...
const disbut = require("discord-buttons");
    ...
    let embed1 = new Discord.MessageEmbed()
      ...
      .setDescription('** *NEKTAR*  is one of the only free all in one bots that has many features like tickets, reaction roles and buttons while keeping the minimalistic feel and look that every bot user loves.**')
      .setColor("#f4c2c2")
      ...
      .setFooter('HEAven Services | Prefix: -')
    ...
    let embed6 = new Discord.MessageEmbed()
     ...
     .MessageEmbed()
      ...
      .addField(
        "<a:pin:908666657520619550> SLASH COMMANDS",
        "`addemoji` `autoplay` `avatar` `ban` `botguild` `botinfo` `channel-info` `channels` `clear` `emojis` `filter` `invite` `join` `joke` `meme` `jump` `mute` `nowplay` `pause` `ping` `play` `resume` `purge` `previous` `clear` `queue` `role-info` `roll` `seek` `server` `shorturl` `shuffle` `skip` `stop` `support` `unban` `uptime` `unmute` `volume`")
